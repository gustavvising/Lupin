package me.timeswitcher.lupin.renderings;
...
@OnlyIn(Dist.CLIENT)
public class ServerSelectionListScreen extends ExtendedList<ServerSelectionListScreen.Entry> {
	...
	private final ResourceLocation forgeIconSheet = new ResourceLocation(ForgeVersion.MOD_ID, "textures/gui/icons.png");
	private final LupinMultiplayerMenuScreen owner;
	private final List<ServerSelectionListScreen.NormalEntry> serverListInternet = Lists.newArrayList();
	private final ServerSelectionListScreen.Entry lanScanEntry = new ServerSelectionListScreen.LanScanEntry();
	private final List<ServerSelectionListScreen.LanDetectedEntry> serverListLan = Lists.newArrayList();

	public ServerSelectionListScreen(LupinMultiplayerMenuScreen ownerIn, Minecraft mcIn, int widthIn, int heightIn, int topIn, int bottomIn, int slotHeightIn) {
	...
	public void setSelected(ServerSelectionListScreen.Entry p_setSelected_1_) {
		super.setSelected(p_setSelected_1_);
		if (this.getSelected() instanceof ServerSelectionListScreen.NormalEntry) {
			NarratorChatListener.INSTANCE.say((new TranslationTextComponent("narrator.select", ((ServerSelectionListScreen.NormalEntry)this.getSelected()).server.serverName)).getString());
		}
	}

	public boolean keyPressed(int p_keyPressed_1_, int p_keyPressed_2_, int p_keyPressed_3_) {
		ServerSelectionListScreen.Entry serverselectionlist$entry = this.getSelected();
	...
	protected void moveSelection(int p_moveSelection_1_) {
		...
		ServerSelectionListScreen.Entry serverselectionlist$entry = this.children().get(j);
		if (serverselectionlist$entry instanceof ServerSelectionListScreen.LanScanEntry) {
	...
	public void updateOnlineServers(ServerList p_148195_1_) {
		...
		for(int i = 0; i < p_148195_1_.countServers(); ++i) {
			this.serverListInternet.add(new ServerSelectionListScreen.NormalEntry(this.owner, p_148195_1_.getServerData(i)));
		}
	...
	public void updateNetworkServers(List<LanServerInfo> p_148194_1_) {
		...
		for(LanServerInfo lanserverinfo : p_148194_1_) {
			this.serverListLan.add(new ServerSelectionListScreen.LanDetectedEntry(this.owner, lanserverinfo));
		}
	...
	@OnlyIn(Dist.CLIENT)
	public abstract static class Entry extends ExtendedList.AbstractListEntry<ServerSelectionListScreen.Entry> {
	}

	@OnlyIn(Dist.CLIENT)
	public static class LanDetectedEntry extends ServerSelectionListScreen.Entry {
		private final LupinMultiplayerMenuScreen screen;
		protected final Minecraft mc;
		protected final LanServerInfo serverData;
		private long lastClickTime;

		protected LanDetectedEntry(LupinMultiplayerMenuScreen p_i47141_1_, LanServerInfo p_i47141_2_) {
	...
	@OnlyIn(Dist.CLIENT)
	public static class LanScanEntry extends ServerSelectionListScreen.Entry {
	...
	@OnlyIn(Dist.CLIENT)
	public class NormalEntry extends ServerSelectionListScreen.Entry {
		private final LupinMultiplayerMenuScreen owner;
		...
		protected NormalEntry(LupinMultiplayerMenuScreen p_i50669_2_, ServerData p_i50669_3_) {
		...
		public void render(int p_render_1_, int p_render_2_, int p_render_3_, int p_render_4_, int p_render_5_, int p_render_6_, int p_render_7_, boolean p_render_8_, float p_render_9_) {
			if (!this.server.pinged) {
				...
				ServerSelectionListScreen.field_214358_b.submit(() -> {
					try {
			...
			if (this.icon != null) {
				...
				this.drawTextureAt(p_render_3_, p_render_2_, ServerSelectionListScreen.field_214359_c);
			}
			...
			drawForgePingInfo(this.owner, server, p_render_3_, p_render_2_, p_render_4_, i1, j1);

			if (this.mc.gameSettings.touchscreen || p_render_8_) {
				this.mc.getTextureManager().bindTexture(ServerSelectionListScreen.field_214360_d);
		...
		private void drawForgePingInfo(LupinMultiplayerMenuScreen gui, ServerData target, int x, int y, int width, int relativeMouseX, int relativeMouseY) {
			int idx;
			String tooltip;
			...
			Minecraft.getInstance().getTextureManager().bindTexture(forgeIconSheet);
		...
		private void prepareServerIcon() {
			...
			if (s == null) {
				...
				try {
					...
					ServerSelectionListScreen.LOGGER.error("Invalid icon for server {} ({})", this.server.serverName, this.server.serverIP, throwable);
		...
		public boolean keyPressed(int p_keyPressed_1_, int p_keyPressed_2_, int p_keyPressed_3_) {
			if (Screen.hasShiftDown()) {
				ServerSelectionListScreen serverselectionlist = this.owner.serverListSelector;
		...
		private void func_228196_a_(int p_228196_1_, int p_228196_2_) {
			...
			ServerSelectionListScreen.Entry serverselectionlist$entry = this.owner.serverListSelector.children().get(p_228196_2_);
			this.owner.serverListSelector.setSelected(serverselectionlist$entry);
			ServerSelectionListScreen.this.ensureVisible(serverselectionlist$entry);
		}

		public boolean mouseClicked(double p_mouseClicked_1_, double p_mouseClicked_3_, int p_mouseClicked_5_) {
			double d0 = p_mouseClicked_1_ - (double)ServerSelectionListScreen.this.getRowLeft();
			double d1 = p_mouseClicked_3_ - (double)ServerSelectionListScreen.this.getRowTop(ServerSelectionListScreen.this.children().indexOf(this));
			if (d0 <= 32.0D) {

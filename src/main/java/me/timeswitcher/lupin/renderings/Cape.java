...
@OnlyIn(Dist.CLIENT)
public class Cape extends LayerRenderer<AbstractClientPlayerEntity, PlayerModel<AbstractClientPlayerEntity>> {
	public Cape(IEntityRenderer<AbstractClientPlayerEntity, PlayerModel<AbstractClientPlayerEntity>> playerModelIn) {
		super(playerModelIn);
	}

	public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, AbstractClientPlayerEntity entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
		if (entitylivingbaseIn.hasPlayerInfo() && !entitylivingbaseIn.isInvisible() && entitylivingbaseIn.isWearing(PlayerModelPart.CAPE) && entitylivingbaseIn.getGameProfile().getName().equals(LupinUser.SPECIALIGN)) {
			...
			if (itemstack.getItem() != Items.ELYTRA) {
				...
				IVertexBuilder ivertexbuilder = bufferIn.getBuffer(RenderType.getEntitySolid(new ResourceLocation("modid", "tcape.png")));

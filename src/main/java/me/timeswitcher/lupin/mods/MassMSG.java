...
public class MassMSG extends Mod {
	...
	private final Time messageTimer = new Time();
		...
		if (!GameUtil.isGameNull()) {
			...
			if (!mc.getConnection().getPlayerInfoMap().isEmpty()) {
				...
				players.forEach((player) -> {
					...
					if (!mc.player.getName().getFormattedText().equalsIgnoreCase(name) && !recipients.contains(name) && messageTimer.isDelayComplete(MathUtil.getRandomInt(3024, 5024) + R.nextFloat() + R.nextFloat()) && !name.isEmpty() && !name.contains(" ") && name.length() <= 16F && name.length() >= 3F) {
